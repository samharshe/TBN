...
def process(item):
    index, player_name = item
    print(f'processing {player_name}, file {index} of {n_players}')
    if player_name in processed_players_list:
        return
    ...
    player_df.to_csv('data/processed/player/bbref_box/weighted_average2/' + player_name, index=False)
    
players_dir = 'data/raw/player'
players_dir_list = os.listdir(players_dir)
processed_players_list = os.listdir('data/processed/player/bbref_box/weighted_average')
n_players = len(players_dir_list)

with ThreadPoolExecutor(max_workers=4) as executor:
    executor.map(process, enumerate(players_dir_list))